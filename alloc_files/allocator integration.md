...
1. `crate::kernel_main_post_heap()` — diverges (`-> !`)
...
1. For Multiboot2: skips regions with `base >= BOOT_MAPPED_PHYS` (not yet mapped)
1. Drops the lock
1. Calls `TLSF.init(TLSF_POOL_ORDER)` — use order 8 (1 MiB) as the default
